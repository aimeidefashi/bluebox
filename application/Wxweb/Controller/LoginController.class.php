...
class LoginController extends HomebaseController {
     ...
     public function login()
	{
		...
		if ($ubeiopenid == '') {
			...
			$url = urlencode($url);
			$wxurl = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=' . $WX_config['APP_ID'] . '&redirect_uri=' . $url . '&response_type=code&scope=snsapi_base#wechat_redirect';
			...
			if ($sysconfig['cdenglucode'] == 1) {
				...
				$url = urlencode($url);
				$wxurl = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=' . $WX_config['APP_ID'] . '&redirect_uri=' . $url . '&response_type=code&scope=snsapi_userinfo#wechat_redirect';
			}
			header('Location:' . $wxurl);
		}
	}
	public function getuopenid()
	{
		...
		$url = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid=' . $WX_config['APP_ID'] . '&secret=' . $WX_config['APP_SECRET'] . '&code=' . $wxcode . '&grant_type=authorization_code';
